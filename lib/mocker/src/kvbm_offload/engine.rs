...
impl MockOffloadEngine {
    ...
    fn external_g2_source_for_hashes(&self, hashes: Vec<SequenceHash>) -> Option<SourceBlocks<G2>> {
        ...
        let blocks: Vec<_> = hashes
            ...
            .map(|block| ExternalBlock::<G2>::new(block.block_id(), block.sequence_hash()))
            .collect();
        if blocks.is_empty() {
            return None;
        }
        // MockWorker only needs block id/hash for timing and lower-tier
        // publication, so background copies should not strong-pin G2 slots.
        Some(SourceBlocks::External(blocks))
    }
...
mod tests {
    ...
    #[test]
    fn g2_to_g3_background_copy_does_not_strong_pin_g2_source() {
        use dynamo_tokens::PositionalLineageHash;
        ...
        assert!(
            engine.g2_manager.allocate_blocks(1).is_some(),
            "in-flight G2→G3 mock copy should not strong-pin the G2 source block"
        );
    }
